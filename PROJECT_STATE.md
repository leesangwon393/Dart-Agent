...
- **업종 확장 라운드에서 새 문제 2건 발견(미수정)**:
  1. 다중행 표 요약 시 답변 모델이 숫자를 잘못 읽음(알테오젠 라이선스
     계약금액을 원문 80억원인데 800억원으로 10배 오답 — validator가
     정확히 잡아낸 진짜 오류, 오탐 아님).
  2. 업종별 회계 용어 차이에 agent가 적응 못함(금융지주는 "매출액" 대신
     "영업수익/영업손익"을 쓰는데 agent가 검색어를 안 바꾸고 동일 쿼리
     4회 반복하다 max_iterations 도달 — 결과는 맞았지만 비효율).
- 미해결로 남은 것(변동 없음): "복합추론_Open"(다년도/장문 비교) 유형이
  top_k=5 retrieval breadth 부족으로 반복 실패 — 여전히 다음 우선순위.
...
- [x] 8개 업종(기존 3개 + 신규 금융보험/바이오제약/조선/자동차/건설/통신
      6개) 10개사로 매트릭스 확장, `results/generalization_check/matrix.csv`
      + `raw_sector_expansion_queries.json`
- [x] 카운팅 오분류 + tool 중복호출 버그 수정 (§10)
- [x] Validator 오탐 2건 수정 (§10)
- [x] 회귀 테스트 3건 추가(`tests/test_agent.py`)

### 아직 미해결(우선순위 순)
1. **복합추론_Open(다년도/장문 비교) retrieval breadth 부족** — 삼성전자
   ...
   통째로 가져오는 전용 tool.
2. **다중행 표 요약 시 답변 모델의 숫자 오독**(신규, 업종확장 라운드에서
   발견) — 알테오젠 라이선스 계약 정리 질의에서 원문 80억원을 800억원으로
   10배 오답. validator 가 정확히 잡아낸 진짜 오류(오탐 아님). 근본 원인
   추정: 여러 계약이 한 표에 몰려있을 때 행을 헷갈리거나 자릿수를 잘못
   읽음. 개선 방향: 답변 생성 시 표 형태 근거는 행 단위로 재구성해서
   넘기거나, 숫자 재확인을 요구하는 프롬프트 보강.
3. **업종별 회계 용어 차이에 agent 가 적응 못함**(신규) — 금융지주는
   "매출액" 대신 "영업수익/영업손익"을 쓰는데 agent 가 검색어를 안 바꾸고
   동일 쿼리 반복하다 max_iterations 도달. 개선 방향: 업종별 동의어 사전
   추가, 또는 검색 실패 시 다른 키워드로 재시도하라는 지침 강화.
