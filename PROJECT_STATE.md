...
- **`query_normalizer.py`에 `[YEAR]` placeholder 추가** (2026-08-26, Entity
  ...
  가능해 보임).
- **[신규, 2026-08-26 파트 2 실검증에서 발견]** `ANSWER_SYSTEM_PROMPT`에
  "이벤트 발생일과 그 이벤트가 참조하는 원본 계약/공시의 체결일·제출일이
  다를 수 있으니 혼동하지 말라"는 원칙 추가 검토 — 실제 사례: "2025년에
  체결한 계약 중 해지된 게 있는가?" 질문에서, evidence에 "정정관련
  공시서류제출일: 2024-10-15"(원계약이 2024년 체결)이 명시돼 있었는데도
  "2025년에 해지 공시가 있었다"는 사실과 "2025년에 체결됐다"는 사실을
  혼동해 오답(§5-D Q5). 검색은 정확했고 순수 답변생성 단계의 논리 오류.
- **[신규, 2026-08-26 파트 2 실검증에서 발견]** `comparison_axis` 계산에서
  ...
  구성요소) period 비교로 세지 않는 정교화 필요 — 이번 6문항에서는 무해
  했지만(Q2 라우팅에 영향 없음) 실제 피해 사례가 나오면 우선순위 올릴 것.
- **[신규, 2026-08-26 파트 2 실검증에서 발견]** 복합문서추론 Open 질문에서
  Agent가 지나치게 일반적인 검색어("사업보고서", "핵심 사업")와 낮은
  top_k(=1)를 써서 실제로 존재하는 답을 못 찾는 사례 재확인(§5-D Q6 —
  같은 인덱스에 "사업의 개요 부문별 매출 비중"으로 직접 BM25 질의하면
  정답 chunk가 1위로 나옴). §11/기존 "복합추론_Open retrieval breadth
  개선" 후보와 같은 계열의 문제 — search_disclosures tool description에
  "일반적인 키워드보다 문서에 실제 나올 법한 구체적 섹션명/문구를 쓰고
  top_k를 5 이상으로 유지하라"는 가이드 보강 검토.
