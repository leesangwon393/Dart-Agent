...
2. **[완료 2026-08-29]** ~~연결/별도 재무제표 혼동~~(SK하이닉스) — 질문에
   ...
   원칙 추가로 수정(§5-H).
3. **[완료 2026-08-29]** ~~10배 단위환산 자기모순 재발~~(아모레퍼시픽) —
   같은 문장 안에서 정답과 10배 축소값을 동시 제시. 알테오젠/레인보우
   로보틱스와 같은 계열의 구조적 문제로 재확인 후 프롬프트+validator로 대응(§5-J).
...
### 5-J. [완료] §7 우선순위 3 — 10배 단위환산 자기모순 근본원인 규명 + 프롬프트/검산 대응 (2026-08-29)

3개사(알테오젠/레인보우로보틱스/아모레퍼시픽)에서 반복 재현된 "10배
단위환산 자기모순"의 근본원인을 실제 로그(matrix.csv, results.json)로
정확히 규명하고 대응했다.

**근본원인**: 답변모델(HCX-005)이 evidence의 "원" 단위 숫자를 사람이
읽기 좋은 "백만원"/"억원" 표기로 자발적으로(프롬프트가 시키지 않았는데도)
암산 재환산하다가 자릿수를 잘못 옮긴다 — "LLM 암산을 신뢰하지 않는다"는
기존 원칙(증가율/비율 계산에 이미 적용됨)이 놓치고 있던 카테고리였다.
실측 2건을 정확히 재현/분석:
- **레인보우로보틱스**: evidence `28,178,420,000원` → 답변 "약
  281,784백만 원"(정확히 100,000분의 1 축소 — 원→백만원 변환 중 자릿수를
  하나 더 밀어서 10배 부풀린 값이 된 경우).
- **아모레퍼시픽**: evidence `335,827백만원`을 답변이 그대로도 쓰고, 같은
  문장에서 "(또는 335억 8,270만원)"으로 다시 풀어 썼는데 이건 evidence
  값의 정확히 1000분의 1 — 두 표기가 서로 모순된다(올바른 재환산은
  "3,358억 2,700만원"이어야 함).

**대응 1 — 프롬프트(예방)**: `ANSWER_SYSTEM_PROMPT` 원칙 9번 추가 —
evidence의 숫자/단위를 그대로 옮기고, 원/천원/백만원/억원 사이 단위
환산을 암산으로 하지 말고, 같은 값을 괄호로 다른 단위로 다시 풀어 쓰지
말라고 명시(실제 재현 사례를 프롬프트에 인용).

**대응 2 — validator(탐지, defense-in-depth)**: `_looks_like_unit_scale_
error()`를 새로 추가 — ungrounded로 판정된 숫자가 evidence 숫자 중
하나의 정확히 10^k배(`k = round(log10(claimed/v))`, `|k|<=9`, 1% 이내
오차)인지 검사한다. 단순 소수의 배수 목록(10/100/1000...)이 아니라
log10 반올림 방식을 쓴 이유: 실제 관찰된 비율이 10배(단순 부풀림)뿐
아니라 100,000배(원→백만원 변환 중 자릿수 하나 추가 오류)처럼 "흔한
단위쌍의 배수가 아닌" 값도 나왔기 때문 — 아무 배수든 10의 거듭제곱이면
잡아낸다. **grounded로 인정하지는 않는다**(실제로 틀린 숫자이므로) —
대신 "[단위환산 오류 의심]"이라는 별도 경고로 "근거가 아예 없음"과
구분해서 원인 추적을 돕는다(`has_citation`의 "손상 vs 누락" 구분과
같은 설계 취지).

**실측 확인**: 위 2건 실제 숫자를 그대로 재현 — 둘 다 `numbers_grounded
=False`(여전히 틀린 숫자로 정확히 처리)이면서 `[단위환산 오류 의심]`
경고가 정확히 뜸을 직접 실행으로 확인. 무관한 숫자는 오탐 안 함(우연히
비율이 안 맞는 케이스로 확인).
...
3. **[완료 2026-08-29]** ~~10배 단위환산 자기모순 재발~~(아모레퍼시픽,
   알테오젠/레인보우로보틱스와 동일 계열) — 근본원인 규명(답변모델이
   evidence의 "원" 단위 숫자를 백만원/억원으로 암산 재환산하다 자릿수를
   잘못 옮김) + `ANSWER_SYSTEM_PROMPT` 원칙 9번(재환산 금지) + validator
   `_looks_like_unit_scale_error()` 진단 추가. §5-J 참고.
